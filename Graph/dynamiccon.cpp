// {1,{a,b}} add edge {2,{a,b}} remove edge
// {3, {0, 0}} count number or components
struct DynamicConnectivity {
  ...
  void go(int l,int r,int n,
          int out,vector<edge>es){
    vector<edge> nes;
    for (int i=1;i<=n;i++) {
      g[i].clear(); id[i]=0; is[i]=0;
    }
    for (auto e:es) {
  ...
  vector<int> solve(int n, vector<pair<int,
         pair<int, int> > > queries) {
    map<pair<int, int>, int> ae;
    tq.resize(queries.size()); id.resize(n+1);
    is.resize(n+1); g.resize(n+1);
    ...
    for (auto e:ae)
      if (e.S) es.push_back({e.F.F, e.F.S,
              e.S-1, (int)queries.size()});
    ret.resize(qs);
